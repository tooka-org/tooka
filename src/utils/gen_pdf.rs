...
impl PDFGenerator {
    ...
    fn render_pages(&mut self, flat_entries: &[(Option<String>, Option<&MatchResult>)]) {
        ...
        for (rule_id_opt, entry_opt) in flat_entries {
            if let Some(rule_id) = rule_id_opt {
                self.page_break_if_needed(min_y + 30.0);
            ...
            if let Some(entry) = entry_opt {
                self.page_break_if_needed(min_y + 50.0);

                if self.y == PAGE_HEIGHT - MARGIN_TOP - 50.0 {
                ...
                self.draw_match_result_block(entry, self.y + 5.0);
                self.y -= 60.0;
    ...
    fn page_break_if_needed(&mut self, min_y: f32) {
        if self.y < min_y {
            self.finish_page();
            self.page_number += 1;
            self.y = PAGE_HEIGHT - MARGIN_TOP - 50.0;
    ...
    fn draw_match_result_block(&mut self, result: &MatchResult, y_top: f32) {
