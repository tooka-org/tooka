...
pub fn sort_files(source: String, rules: String, dry_run: bool) -> Result<Vec<MatchResult>, String> {
    log::debug!("Starting file sorting with source: '{}', rules: '{}', dry_run: {}", source, rules, dry_run);
    ...
    if !source_path.exists() || !source_path.is_dir() {
        log::error!("Source path '{}' does not exist or is not a directory.", source_path.display());
    ...
    let rules_file = if rules == "<all>" {
        let rf = rules::load_rules().map_err(|e| e.to_string()).expect("Failed to load rules");
        ...
        rules::load_rules_from_ids(rule_ids).map_err(|e| e.to_string()).expect("Failed to load rules from IDs")
    };
    log::debug!("Loaded {:?} rules", rules_file.rules.len());

    let entries: Vec<PathBuf> = source_path.read_dir()
        .map_err(|e| e.to_string()).expect("Failed to read source directory")
    ...
    let results: Result<Vec<_>, _> = entries.par_iter()
        ...
        .collect();
    log::debug!("File sorting completed with {} results.", results.as_ref().map_or(0, |r| r.len()));
